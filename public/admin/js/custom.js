...
function template_functions(){
	...
	$('#myTab_1 a:first').tab('show');
	$('#myTab_1 a').click(function (e) {
	  e.preventDefault();
	  $(this).tab('show');
	});
	/*-------------------------- Kéo rê di chuyển các box--------------------------------*/
	// $('.sortable').sortable({
	// 	revert:true,
	...
	/* ---------- Progress  ---------- */
		...
		var data_progress =$('.progressAnimateValue').attr('data-progress');
		$(".progressAnimate").progressbar({
			value: 1,
			create: function() {
				$(".progressAnimate .ui-progressbar-value").animate({"width":data_progress},{
					duration: 3000,
					step: function(now){
...
$(document).ready(function(){
	if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
 		$('.detect-mobile').html('Mobile Detect');
	}
	else{
		// console.log('PC');
	}
...
function setTodayCalendar(){
	let setdate = $('td.fc-day-number.fc-today').html();
	setdate = '<span style="margin-right:28px">Hôm nay</span>'+ setdate;
	$('td.fc-day-number.fc-today').html(setdate);
}
...
function fadeOutAlert(className,timeOut = 5000){
	setTimeout(function() {
		$(className).fadeTo(500, 0).slideUp(500, function(){
			$(this).remove(); 
		});
	}, timeOut);
};
//public function fadeOutAlert
window.fadeOutAlert = fadeOutAlert;
